...
**[English README](README.md)** | [快速开始](#快速开始) | [模型性能](#模型性能) | [致谢](#致谢)
...
*测试集 5,704 场 · 时序切分 70/15/15 · 无需form/streak/career数据*

**模式选择**：
- ⚡ **Quick**：只需排名、ELO、国籍、赛事信息 → 30秒出结果（AUC 0.87）
- 🔬 **Expert**：需要完整数据包括近期状态 → 最高精度（AUC 0.96）
...
| 模式 | 输入 | 特征数 | 模型 | 性能 | 适合场景 |
|------|------|--------|------|------|---------|
| ⚡ **Quick** | 排名、ELO、国籍、赛事级别、轮次、主场 | 21 | QuickEnsemble | AUC 0.87, Acc 81% | 快速判断，30秒出结果 |
| 🔬 **Expert** | 所有字段必填：排名、ELO、国籍、赛事级别、轮次、举办国、近况（5/10/20场）、H2H、连胜/败、生涯场次、三局比例 | 35 | SimplifiedEnsemble | AUC 0.96, Acc 90% | 最高精度 |

**重要**：两个模式现在都使用专用训练的模型，Quick模式不再使用默认值填充缺失特征。

**数据查询**：
- **所有数据均来自 [BadmintonRanks.com](https://badmintonranks.com)**
- 排名：球员页面 → "档案资料" 标签页（显示当前排名）
- ELO 评分：球员页面 → "比赛详情" 标签页
- 近期战绩（5/10/20场）：球员页面 → "比赛详情" 标签页
- 连胜/败：球员页面 → "连胜场次" 标签页
- 交手记录（H2H）：球员页面 → "交手记录" 标签页
- 生涯场次：球员页面 → "档案资料" 标签页（显示生涯胜负，如"453-81"）
...
├── scripts/
│   ├── train_simplified.py        # 训练Expert模式模型（35特征）
│   ├── train_quick.py             # 训练Quick模式模型（21特征）
...
**本项目的比赛数据来自 [BadmintonRanks.com](https://badmintonranks.com/)**

非常感谢 BadmintonRanks.com 的 owner 慷慨授权使用其数据库用于这个研究项目。
BadmintonRanks.com 提供了完整的比赛记录（比分、对阵、赛事级别等），
本项目在此基础上计算了 MOV ELO 评分和所有高级特征。
</div>
