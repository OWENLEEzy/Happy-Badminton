...
SET_COUNT_FEATURES = [
    ...
    "h2h_win_rate_bayes",
    # Career stage (winner only; career_stage_l excluded — cross-validation showed
    # loser's career stage does not improve set count AUC)
    "career_stage",
    # Interactions
    "rank_x_form_diff",
...
def prepare_data(config: dict) -> tuple:
    ...
    logger.info("=" * 60)
    logger.info("Preparing set count model training data")
    logger.info("=" * 60)
    ...
    logger.info(
        f"Train set distribution — 2-set: {(y_train == 0).sum()}, 3-set: {(y_train == 1).sum()}"
    )
...
def main() -> None:
    logger.info("")
    logger.info("╔════════════════════════════════════════╗")
    logger.info("║   Set Count Model Training             ║")
    logger.info("╚════════════════════════════════════════╝")
    logger.info("")
    ...
    logger.info("Training StackingEnsemble (set count)...")
    ...
    logger.info("")
    logger.info("╔════════════════════════════════════════╗")
    logger.info("║   Set Count Model Performance          ║")
    logger.info("╠════════════════════════════════════════╣")
    ...
    model_path = models_dir / "set_count_model.pkl"
    ...
    results_path = models_dir / "set_count_results.json"
    with open(results_path, "w") as f:
        json.dump(results, f, indent=2)
    logger.info(f"Results saved: {results_path}")
    logger.info("")
    logger.info("Set count model training complete!")
